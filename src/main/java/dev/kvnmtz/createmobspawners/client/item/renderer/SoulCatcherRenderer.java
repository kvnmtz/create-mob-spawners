package dev.kvnmtz.createmobspawners.client.item.renderer;
...
public class SoulCatcherRenderer extends CustomRenderedItemModelRenderer {
    ...
    private static final PartialModel GEAR
            = PartialModel.of(CreateMobSpawnersMod.asResource("item/soul_catcher/gear"));
    private static final PartialModel GEAR_EMPTY
            = PartialModel.of(CreateMobSpawnersMod.asResource("item/soul_catcher/gear_empty"));

    @Override
    protected void render(ItemStack stack, CustomRenderedItemModel model,
                          PartialItemModelRenderer renderer, ItemDisplayContext itemDisplayContext,
                          PoseStack ms, MultiBufferSource multiBufferSource, int light, int overlay) {
        renderer.render(model.getOriginalModel(), light);

        if (stack.is(ModItems.EMPTY_SOUL_CATCHER)) {
